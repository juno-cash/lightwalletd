...
// Juno Cash: saplingCommitmentTreeSize is always 0 (Sapling not supported).
type ChainMetadata struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	SaplingCommitmentTreeSize uint32 `protobuf:"varint,1,opt,name=saplingCommitmentTreeSize,proto3" json:"saplingCommitmentTreeSize,omitempty"` // Always 0 for Juno Cash (Sapling not supported)
...
// A compact representation of a Juno Cash transaction.
//
...
// related to your wallet (for coinbase/mining only in Juno Cash).
...
type CompactTx struct {
	...
	Spends  []*CompactSaplingSpend  `protobuf:"bytes,4,rep,name=spends,proto3" json:"spends,omitempty"`   // Always empty for Juno Cash (Sapling not supported)
	Outputs []*CompactSaplingOutput `protobuf:"bytes,5,rep,name=outputs,proto3" json:"outputs,omitempty"` // Always empty for Juno Cash (Sapling not supported)
...
// UNUSED in Juno Cash (Sapling not supported). Kept for API compatibility.
// A compact representation of a Sapling Spend Description.
type CompactSaplingSpend struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Nf []byte `protobuf:"bytes,1,opt,name=nf,proto3" json:"nf,omitempty"` // Nullifier
}
...
// UNUSED in Juno Cash (Sapling not supported). Kept for API compatibility.
// A compact representation of a Sapling Output Description.
